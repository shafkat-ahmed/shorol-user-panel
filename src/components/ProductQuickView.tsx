...
const ProductQuickView: FC<ProductQuickViewProps> = ({ className = "",selectedProduct,activeIndex }) => {
  const { sizes, variants, status, allOfSizes } = PRODUCTS[0];
  const LIST_IMAGES_DEMO = [detail1JPG, detail2JPG, detail3JPG];

  const [variantActive, setVariantActive] = useState(activeIndex);
        ...
        <label className="rtl:text-right block" htmlFor="">
          <span className="text-sm font-medium">
            ...
            <span className="ms-1 font-semibold">
              {selectedProduct.inventoryInfos[variantActive]?.color?.name}
            </span>
          </span>
        </label>
        <div className="flex mt-2.5">
          {selectedProduct.inventoryInfos?.map((variant:any, index:number) => (
            <div
            key={index}
      ...
      <div className="space-y-8">
        ...
        <div>
          <h2 className="text-2xl font-semibold hover:text-primary-6000 transition-colors">
            <Link href="/product-detail">{selectedProduct.name}</Link>
          </h2>
          ...
          <div className="flex justify-start rtl:justify-end items-center mt-5 space-x-4 sm:space-x-5 rtl:space-x-reverse">
            ...
            <Prices
              ...
              price={selectedProduct.inventoryInfos[variantActive]?.price}
            />
          ...
          <div className="relative">
            <div className="aspect-w-16 aspect-h-16">
              <Image
                src=
                {
                  selectedProduct.inventoryInfos[variantActive]?.image?
                  getImageUrl(selectedProduct.inventoryInfos[variantActive].image?.url)
                  :
                  getImageUrl(selectedProduct.mainImage?.url)
                }
                fill
